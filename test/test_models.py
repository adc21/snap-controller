...
DamperCatalog, CaseTemplate の各データモデルを包括的にテストします。
...
# DamperCatalog
# ---------------------------------------------------------------------------
from app.models.damper_catalog import DamperCatalog, DamperSpec, get_catalog


class TestDamperCatalog:
    """DamperCatalog のユニットテスト。"""

    def test_builtin_catalog(self):
        catalog = DamperCatalog()
        specs = catalog.all_specs
        assert len(specs) >= 9

    def test_get_by_id(self):
        catalog = DamperCatalog()
        spec = catalog.get_by_id("oil_standard_500")
        assert spec is not None
        assert "500" in spec.name

    def test_get_by_category(self):
        catalog = DamperCatalog()
        oil = catalog.get_by_category("oil")
        assert len(oil) >= 3
        for s in oil:
            assert s.category == "oil"

    def test_get_by_keyword(self):
        catalog = DamperCatalog()
        dvod = catalog.get_by_keyword("DVOD")
        assert len(dvod) >= 3

    def test_search(self):
        catalog = DamperCatalog()
        results = catalog.search("鋼材")
        assert len(results) >= 2

    def test_add_custom(self):
        catalog = DamperCatalog()
        before = len(catalog.all_specs)
        custom = DamperSpec(id="custom_test", name="カスタムダンパー", category="oil")
        catalog.add_custom(custom)
        assert len(catalog.all_specs) == before + 1
        assert catalog.get_by_id("custom_test").is_custom is True

    def test_remove_custom(self):
        catalog = DamperCatalog()
        custom = DamperSpec(id="custom_rm", name="削除テスト", category="oil")
        catalog.add_custom(custom)
        assert catalog.remove_custom("custom_rm") is True

    def test_remove_builtin_fails(self):
        catalog = DamperCatalog()
        assert catalog.remove_custom("oil_standard_500") is False

    def test_save_and_load_custom(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            catalog = DamperCatalog()
            custom = DamperSpec(id="cst1", name="保存テスト", category="oil")
            catalog.add_custom(custom)

            path = str(Path(tmpdir) / "custom_dampers.json")
            catalog.save_custom(path)

            catalog2 = DamperCatalog()
            loaded = catalog2.load_custom(path)
            assert loaded == 1
            assert catalog2.get_by_id("cst1") is not None

    def test_apply_to_case_params(self):
        catalog = DamperCatalog()
        spec = catalog.get_by_id("oil_standard_500")
        params = catalog.apply_to_case_params(spec)
        assert isinstance(params, dict)
        assert "8" in params  # 減衰係数
    ...
    def test_spec_serialization(self):
        spec = DamperSpec(
            id="test", name="テスト", category="oil",
            snap_keyword="DVOD", parameters={"8": "300"},
        )
        d = spec.to_dict()
        restored = DamperSpec.from_dict(d)
        assert restored.id == "test"
        assert restored.parameters == {"8": "300"}
...
from app.models.case_template import (
    CaseTemplate, TemplateManager, get_builtin_templates,
)


class TestCaseTemplate:
    """CaseTemplate のユニットテスト。"""

    def test_default_creation(self):
        tpl = CaseTemplate()
        assert tpl.name == "新規テンプレート"
        assert tpl.category == "共通"
    ...
    def test_serialization(self):
        tpl = CaseTemplate(
            ...
            category="制振",
            parameters={"DAMPING": 0.05},
            damper_params={"Cd": 500},
            tags=["tag1", "tag2"],
        )
        d = tpl.to_dict()
        restored = CaseTemplate.from_dict(d)
        assert restored.name == "テスト"
        assert restored.parameters == {"DAMPING": 0.05}
        assert restored.tags == ["tag1", "tag2"]

    def test_save_and_load(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tpl = CaseTemplate(name="SaveTest", parameters={"K": 100})
            path = str(Path(tmpdir) / "test.snaptemplate")
            tpl.save(path)

            loaded = CaseTemplate.load(path)
            assert loaded.name == "SaveTest"
            assert loaded.parameters == {"K": 100}

    def test_from_case(self):
        case = AnalysisCase(
            name="Case1",
            parameters={"DT": 0.01},
            damper_params={"Cd": 300},
        )
        tpl = TemplateManager.from_case(case, category="制振")
        assert "Case1" in tpl.name
        assert tpl.parameters == {"DT": 0.01}
        assert tpl.damper_params == {"Cd": 300}
        assert tpl.category == "制振"

    def test_builtin_templates(self):
        builtins = get_builtin_templates()
        assert len(builtins) >= 6
        categories = set(t.category for t in builtins)
        assert "制振" in categories
        assert "免震" in categories


class TestTemplateManager:
    """TemplateManager のユニットテスト。"""

    def test_add_and_list(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            mgr = TemplateManager(template_dir=tmpdir)
            tpl = CaseTemplate(name="AddTest")
            mgr.add(tpl)
            all_templates = mgr.list_all()
            assert len(all_templates) == 1
            assert all_templates[0].name == "AddTest"

    def test_remove(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            mgr = TemplateManager(template_dir=tmpdir)
            tpl = CaseTemplate(name="RemoveTest")
            mgr.add(tpl)
            assert mgr.remove("RemoveTest") is True
    ...
    def test_search(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            mgr = TemplateManager(template_dir=tmpdir)
            mgr.add(CaseTemplate(name="Oil Damper", tags=["oil"]))
            mgr.add(CaseTemplate(name="Steel Damper", tags=["steel"]))
            results = mgr.search("oil")
            assert len(results) == 1
            assert results[0].name == "Oil Damper"

    def test_get_by_category(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            mgr = TemplateManager(template_dir=tmpdir)
            mgr.add(CaseTemplate(name="T1", category="制振"))
            mgr.add(CaseTemplate(name="T2", category="免震"))
            seishin = mgr.get_by_category("制振")
            assert len(seishin) == 1

    def test_update(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            mgr = TemplateManager(template_dir=tmpdir)
            tpl = CaseTemplate(name="UpdateTest", parameters={"K": 100})
            mgr.add(tpl)

            updated = CaseTemplate(name="UpdateTest", parameters={"K": 200})
            result = mgr.update(updated)
            assert result is not None

            mgr.refresh()
            found = mgr.search("UpdateTest")
            assert len(found) == 1
            assert found[0].parameters == {"K": 200}
