...
SNAP_INPUT_FILE_PATH = "test_wind.s8i"

# 結果として参照したいデータを設定（設定できる値は、controller/result.pyのResultConfigDictを参照）
dis_result_config: ResultConfig = CreateResultConfig(15).getAllStoryDVAR("Dx", 39)
acc_result_config: ResultConfig = CreateResultConfig(15).getAllStoryDVAR("Ax", 39)
...
for i in range(10):
    ...
    max_results.append({
        "id": i + 1,
        "k": k,
        "Td": 2 * math.pi * math.sqrt(100000 / (k * 1000)),
        "dis_max": dis_max,
        "acc_max": acc_max,
    })
    print("results")
    ...
    update_config: UpdateConfig = createUpdateConfig(76, k)
