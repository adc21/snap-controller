...
def _sanitize_for_filename(name: str) -> str:
    ...
    アンダースコアに置換します。さらに ``.`` もアンダースコアに
    置換します — SNAP.exe が入力ファイル名中の ``.`` を拡張子区切り
    と誤認して出力フォルダ名を先頭で切り詰めるため、スイープケース
    (例: ``C1.質量 (t)=0.01``) で全ケースが同一出力フォルダに衝突
    してしまう問題を回避します。長さも制限 (48 文字) します。
    """
    if not name:
        return "case"
    forbidden = '\\/:*?"<>|\t\r\n .'
