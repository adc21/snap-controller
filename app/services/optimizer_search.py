...
@dataclass
class _GAParams:
    """GA世代ループで使用するハイパーパラメータ群。"""

    crossover_rate_init: float = 0.8
...
class _SearchAlgorithmsMixin:
    ...
    @staticmethod
    def _make_decode(config: OptimizationConfig) -> Callable[[np.ndarray], Dict[str, float]]:
        """[0,1]正規化ベクトル → パラメータ辞書 変換関数を生成。"""
    ...
    def _make_evaluate(
        self,
        config: OptimizationConfig,
        decode: Callable[[np.ndarray], Dict[str, float]],
    ) -> Callable[[np.ndarray, int], Any]:
        """正規化ベクトル+iteration → OptimizationCandidate 評価関数を生成。"""
        def _evaluate_vec(vec: np.ndarray, iteration: int):
            params = decode(vec)
            response = self._evaluate_fn(params)
            obj_val = config.compute_objective(response, params)
            is_feasible, margins = self._check_constraints(response, config)
        ...
        return _evaluate_vec

    def _make_fitness(self, config: OptimizationConfig) -> Callable[[Any], float]:
        """候補 → スカラー適応度 変換関数を生成。"""
        def _fitness(c) -> float:
    ...
    def _run_ga_search(self, config: OptimizationConfig) -> OptimizationResult:
        ...
        stagnation_limit = max(3, n_generations // 4)

        all_candidates: List[OptimizationCandidate] = []
        best: Optional[OptimizationCandidate] = None

        decode = self._make_decode(config)
        evaluate = self._make_evaluate(config, decode)
        fitness = self._make_fitness(config)

        # 初期集団生成（LHS + ウォームスタート）
        population, warm_injected = self._ga_init_population(
            config, pop_size, n_params,
        )
        pop_candidates: list = []
        for i, chromo in enumerate(population):
            if self._cancelled:
                break
            cand = evaluate(chromo, i)
        ...
        for gen in range(1, n_generations):
            ...
            population, pop_candidates, best = self._ga_generation(
                config, gen, n_generations, n_params, pop_size, n_elite,
                population, pop_candidates, all_candidates,
                best, ga_params, evaluate, fitness,
            )
    ...
    def _ga_init_population(
        ...
        if config.warm_start_candidates:
            ...
            for wc in warm_sorted[:pop_size // 2]:
                ...
                warm_injected += 1
        return population, warm_injected
    ...
    def _ga_generation(
        ...
        """GA: 1世代の選択・交叉・突然変異・評価を実行。"""
        gen_ratio = gen / max(1, n_generations - 1)
        if config.ga_adaptive_mutation:
            mutation_rate = ga_params.mutation_rate_init * (1.0 - 0.7 * gen_ratio)
            mutation_sigma = ga_params.mutation_sigma_init * (1.0 - 0.6 * gen_ratio)
            crossover_rate = (ga_params.crossover_rate_init
                              + (1.0 - ga_params.crossover_rate_init) * gen_ratio * 0.5)
        else:
            mutation_rate = ga_params.mutation_rate_init
            mutation_sigma = ga_params.mutation_sigma_init
            crossover_rate = ga_params.crossover_rate_init

        # エリート選択
        sorted_indices = sorted(range(pop_size), key=lambda i: fitness(pop_candidates[i]))
        ...
        for k in range(n_elite, pop_size):
            if self._cancelled:
                break
            child = self._ga_breed_one(
                population, pop_candidates, pop_size, n_params,
                ga_params.tournament_size, crossover_rate,
                mutation_rate, mutation_sigma, ga_params.blx_alpha, fitness,
            )
            iteration = gen * pop_size + k
            cand = evaluate(child, iteration)
        ...
        return new_population, new_candidates, best
    ...
    def _ga_breed_one(
        ...
        """GA: トーナメント選択 + BLX-α交叉 + ガウシアン突然変異で子個体を生成。"""
        # トーナメント選択
        t1 = random.sample(range(pop_size), tournament_size)
        p1_idx = min(t1, key=lambda i: fitness(pop_candidates[i]))
        t2 = random.sample(range(pop_size), tournament_size)
        p2_idx = min(t2, key=lambda i: fitness(pop_candidates[i]))
    ...
    def _run_sa_search(self, config: OptimizationConfig) -> OptimizationResult:
        ...
        stagnation_limit = max(50, total // 4)

        decode = self._make_decode(config)
        evaluate = self._make_evaluate(config, decode)

        def _cost(cand) -> float:
            if config.constraint_penalty_weight > 0:
                return self._penalized_objective(
                    cand.objective_value, cand.constraint_margins, config,
                )
            if not cand.is_feasible:
                return cand.objective_value + 1e10
            return cand.objective_value

        # 初期解
        current_x = self._sa_init_solution(config, n_params)
        current_cand = evaluate(current_x, 0)
        all_candidates: List[OptimizationCandidate] = [current_cand]
        ...
        # メインループ
        n_accept, no_improve_count = self._sa_anneal_loop(
            config, total, n_params, T_init, cooling_rate, step_size,
            stagnation_limit, current_x, current_cost, best, best_cost,
            all_candidates, evaluate, _cost,
        )
        best = min(all_candidates, key=lambda c: _cost(c)) if all_candidates else None
    ...
    @staticmethod
    def _sa_init_solution(
        config: OptimizationConfig, n_params: int,
    ) -> np.ndarray:
        """SA: ウォームスタートまたはランダムで初期解を生成。"""
        if config.warm_start_candidates:
            warm_sorted = sorted(
                [wc for wc in config.warm_start_candidates
                 if all(k in wc.params for k in [pr.key for pr in config.parameters])],
        ...
        return np.random.rand(n_params)
    ...
    def _sa_anneal_loop(
        ...
        """SA: メトロポリスループ。(n_accept, no_improve_count) を返す。"""
        T = T_init
        n_accept = 0
        no_improve_count = 0
        ...
        for i in range(1, total):
            ...
            if new_cost < best_cost and cand.is_feasible:
                best = cand
                best_cost = new_cost
                no_improve_count = 0
            else:
        ...
        return n_accept, no_improve_count
    ...
    def _run_de_search(self, config: OptimizationConfig) -> OptimizationResult:
        ...
        jDE自己適応 (Brest et al., 2006) — 個体ごとにF, CRを適応。
        """
        if not config.parameters:
            return OptimizationResult(message="探索パラメータが設定されていません。")
        ...
        F_init, CR_init = 0.5, 0.9

        evaluate = self._make_evaluate(config, self._make_decode(config))
        fitness = self._make_fitness(config)

        # 初期集団（GA共通ヘルパー）
        population, warm_injected = self._ga_init_population(config, pop_size, n_params)
        pop_candidates: List[Optional[Any]] = [None] * pop_size
        ...
        for gen in range(1, n_generations):
            ...
            # 1世代の突然変異・交叉・選択
            best = self._de_generation(
                config, population, pop_candidates, F_arr, CR_arr,
                pop_size, n_params, gen, all_candidates, best,
                evaluate, fitness, F_init, CR_init,
            )
            ...
            # 多様性喪失リスタート
            restarted = self._de_maybe_restart(
                population, pop_candidates, F_arr, CR_arr,
                pop_size, n_params, gen, n_generations, n_restarts,
                diversity, all_candidates, best, evaluate, fitness,
                F_init, CR_init,
            )
            if restarted is not None:
                best = restarted[0]
                n_restarts = restarted[1]
                no_improve_gens = 0
                continue
    ...
    def _de_generation(
        ...
        """DE: 1世代のjDE突然変異・二項交叉・貪欲選択を実行。"""
        tau1, tau2 = 0.1, 0.1
        for i in range(pop_size):
            if self._cancelled:
                break
            # jDE: F, CR の自己適応
            F_i = random.uniform(0.1, 1.0) if random.random() < tau1 else F_arr[i]
            CR_i = random.random() if random.random() < tau2 else CR_arr[i]
            ...
            mutant = np.clip(
                population[r1] + F_i * (population[r2] - population[r3]), 0.0, 1.0,
            )
            ...
            trial_cand = evaluate(trial, iteration)
            all_candidates.append(trial_cand)
            self.candidate_found.emit(trial_cand)

            if fitness(trial_cand) <= fitness(pop_candidates[i]):
                population[i] = trial
                pop_candidates[i] = trial_cand
                F_arr[i] = F_i
                CR_arr[i] = CR_i

            if best is None or fitness(trial_cand) < fitness(best):
                best = trial_cand

        return best

    def _de_maybe_restart(
        self,
        population: np.ndarray,
        pop_candidates: list,
        F_arr: np.ndarray,
        ...
        """DE: 多様性喪失時にリスタート。成功時は (best, n_restarts) を返す。"""
        if not (diversity < diversity_threshold
        ...
        ranked = sorted(
            range(pop_size),
            key=lambda k: fitness(pop_candidates[k]) if pop_candidates[k] else float("inf"),
        )
        n_keep = pop_size // 2
        new_pop = self._latin_hypercube_sample(pop_size - n_keep, n_params)
        for idx_new, idx_old in enumerate(ranked[n_keep:]):
            population[idx_old] = new_pop[idx_new]
            pop_candidates[idx_old] = None
            F_arr[idx_old] = F_init
            CR_arr[idx_old] = CR_init
        for idx_old in ranked[n_keep:]:
            if self._cancelled:
                break
            cand = evaluate(population[idx_old], gen * pop_size + idx_old)
